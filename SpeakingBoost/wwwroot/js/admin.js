/**
 * admin.js — Shared utilities for ALL Admin pages
 * Provides: API helper, Toast, Confirm dialog, Sidebar/Topbar render
 */

// ─── 1. API Helper (auto Bearer token) ──────────────────────────────
const Api = {
    _h(extra = {}) {
...
const Toast = {
    ...
    _init() {
        ...
        w.id = 'toastWrap';
        w.style.cssText = 'position:fixed;top:1.25rem;right:1.25rem;z-index:9999;display:flex;flex-direction:column;gap:.5rem;';
...
function adminConfirm(msg) {
    return new Promise(resolve => {
        ...
        if (!el) {
            ...
            el.id = id; el.className = 'modal fade';
            el.innerHTML = `<div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content border-0 shadow" style="border-radius:16px;">
                    <div class="modal-body p-4 text-center">
                        ...
                        <p class="mb-0" id="confirmMsg" style="font-size:.9rem;color:#374151;"></p>
                    </div>
...
function renderAdminSidebar(activeId = '') {
    ...
    html += `<div class="sidebar-footer">
        <div class="user-card" onclick="window.location.href='/admin/profile.html'">
    ...
    return `<nav id="sidebar">${html}</nav>`;
}

// ─── 5. Topbar HTML (no avatar — avatar is sidebar only) ─────────────
...
function deadlineBadge(isOverdue) {
    return isOverdue
        ? `<span class="pill-badge" style="background:#fee2e2;color:#dc2626;">Quá hạn</span>`
        : `<span class="pill-badge" style="background:#dcfce7;color:#16a34a;">Đang mở</span>`;
}
